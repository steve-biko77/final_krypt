...
const buttonVariants = cva(
  // Refonte frontend (partie 1/4) — micro-interaction commune à tous les
  // boutons cliquables : légère élévation au survol (translateY -2px, ~150ms).
  // Réinitialisée pour le variant "link" ci-dessous (un lien texte ne doit pas
  // "décoller" au survol).
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-150 hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-50 disabled:hover:translate-y-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        // Refonte frontend (partie 2/4) — bleu "action principale" du design
        // system (part 1/4), formalisé en variant plutôt que répété en
        // classNames ad-hoc à chaque page restylée (landing, auth, 2FA...).
        brand: "bg-blue-600 text-white hover:bg-blue-700",
      },
      size: {
        default: "h-9 px-4 py-2 has-[>svg]:px-3",
        sm: "h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5",
        // h-11 (44px) — taille tactile minimale mobile (point 5, partie 2/4).
